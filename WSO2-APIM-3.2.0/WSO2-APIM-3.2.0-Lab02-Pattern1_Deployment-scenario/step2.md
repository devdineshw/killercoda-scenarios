...
- Open the custom nging configuration file

`vi apim-nginx.conf`{{execute}}

 - update the host name placeholders with this playground host. Copy only the hostname from below URLs (without https://)
...
`vi /etc/nginx/nginx.conf`{{execute}}
 ...
 - Look for the following key and update accordingly
